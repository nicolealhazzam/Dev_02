...
class Robot:
    ...
    """
    _directions=('nord', 'est', 'sud', 'ouest') # Direction 
    ...
    def __init__(s, x, y, direction):
        """ Initialiser le robot s à partir de sa position (x, y) et sa direction. """
        s.x = x
        s.y = y
        s.direction = Robot._idDirection(direction)

    # l'utilisation de _dx et _dx facilite cette méthode
    def avancer(s):
        """ Avance une case dans la direction. """
        s.x += Robot._dx[s.direction]
        s.y += Robot._dy[s.direction]

    def pivoter(s):
        """ Pivote ce robot de 90° vers la droite. """
        s.direction = (s.direction + 1) % 4

    def afficher(s, prefix=''):
        print(f"{prefix}Robot(x={s.x}, y={s.y}, direction={Robot._nomDirection(s.direction)})")
